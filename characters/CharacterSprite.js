function CharacterSprite(
  ...
  this.feetBox = {
    width: Math.floor(this.width / 2) + this.feetOffset.height,
...
CharacterSprite.prototype = {
  draw: function () {
    ...
    context.beginPath();
    context.rect(
      this.feetBox.position.x,
      this.feetBox.position.y,
      this.feetBox.width,
      this.feetBox.height
    );
    context.stroke();
    ...
    //context.stroke();
    const feetBoxMidPoint = new Vector(
      this.feetBox.position.x + this.feetBox.width / 2,
      this.feetBox.position.y + this.feetBox.height / 2
    );

    const castingVector = new Vector(this.velocity.x, this.velocity.y);
    castingVector.scale(4);
    const locationCheck = new Vector(
      feetBoxMidPoint.x,
      this.feetBox.position.y
    );
